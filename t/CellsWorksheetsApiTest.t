...
#
# cells_worksheets_delete_worksheets test ?
#
{    
    my $name = $BOOK1; # replace NULL with a proper value
    my $folder = $TEMPFOLDER; # replace NULL with a proper value
	ready_file('api'=> $api, 'file'=>$BOOK1 ,'folder' =>$TEMPFOLDER) ;   
    my $all_sheets =  ["Sheet1", "Sheet2" ];
    my $match_condition = AsposeCellsCloud::Object::MatchConditionRequest->new (FullMatchConditions =>  $all_sheets);
    $result = $api->cells_worksheets_delete_worksheets(name => $name,match_condition => $match_condition );
    ok($result->status eq 'OK' ,'cells_worksheets_put_worksheet_background OK');
}

#
# cells_worksheets_get_page_count test ?
#
{    
    my $name = $BOOK1; # replace NULL with a proper value
    my $folder = $TEMPFOLDER; # replace NULL with a proper value
