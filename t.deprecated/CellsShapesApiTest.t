...
#
# cells_shapes_post_worksheet_ungroup_shape test
#
{
    my $name = $BOOK1; # replace NULL with a proper value
    ...
    my $folder = $TEMPFOLDER; # replace NULL with a proper value
    ready_file('api'=> $api, 'file'=>$name ,'folder' =>$folder) ;  
    $result = $api->cells_shapes_post_worksheet_ungroup_shape(name => $name, sheet_name => $sheet_name, shapeindex => 6, folder => $folder);
    ok($result->status eq 'OK' ,'cells_shapes_post_worksheet_ungroup_shape OK');
}

#
# cells_shapes_post_worksheet_group_shape test
#
{
    my $name = $BOOK1; # replace NULL with a proper value
    my $sheet_name = $SHEET6; # replace NULL with a proper value
    ...
    my $list_shape = "[1,3]";
    ready_file('api'=> $api, 'file'=>$name ,'folder' =>$folder) ;  
    $result = $api->cells_shapes_post_worksheet_group_shape(name => $name, sheet_name => $sheet_name, list_shape =>$list_shape, folder => $folder);
    ok($result->status eq 'OK' ,'cells_shapes_post_worksheet_group_shape OK');
}
